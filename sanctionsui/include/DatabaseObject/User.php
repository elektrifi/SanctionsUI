<?php
    class DatabaseObject_User extends DatabaseObject
    {
        ...
        {
            $this->_newPassword = Text_Password::create(10, 'unpronounceable', 'alphanumeric');
        ...
        public function sendEmail($tpl)
        {
            ...
            // JF's alternative
 			// $from = Zend_Registry::get('config')->email->from->email . ' ' . Zend_Registry::get('config')->email->from->name;
 			//$to = addTo($this->profile->email,trim($this->profile->first_name . ' ' . $this->profile->last_name)); 			
 			//$to = $this->profile->email;
 			$host = Zend_Registry::get('config')->email->host;  			
 			$port = Zend_Registry::get('config')->email->smtp_port;
 			$username = Zend_Registry::get('config')->email->smtp_username;
 			$password = Zend_Registry::get('config')->email->smtp_password;
			...
			$settings = array(	'auth' => 'login',
                                ...
                                'username' => $username,
                                'password' => $password);
            $transport = new Zend_Mail_Transport_Smtp($host, $settings);
            $email_from = Zend_Registry::get('config')->email->from->email;
            $name_from =  Zend_Registry::get('config')->email->from->name;
            $email_to = $this->profile->email;
            $name_to = trim($this->profile->first_name . ' ' . $this->profile->last_name);
